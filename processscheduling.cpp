...
void algo::fcfs()
{
    ...
    for (int i = 0; i < n; i++)
    {
        cout << "P" << a[i].idx << "\t" << a[i].atm << "\t" << a[i].bt << "\t" << (a[i].ct) << "\t" << a[i].tat << "\t" << a[i].wt << endl;
        tot_wt += a[i].wt;
        tot_tat += a[i].tat;
    }
    cout << "Avg turn around time of the process =" << (float)(tot_tat / n) << endl;
    cout << "Avg waiting time of the process is =" << (float)(tot_wt / n) << endl;
}
void algo::srtf()
{
...
void algo::sjf()
{
    ...
    while (complete != n)
    {
        for (int i = 0; i < n; i++)
        {
            if (a[i].atm <= t && a[i].rt < minm && a[i].rt > 0)
            {
                minm = a[i].rt;
                shortest = i;
                check = true;
        ...
        if (a[shortest].rt == 0)
        {
            ...
            a[shortest].wt = finish_time - a[shortest].bt - a[shortest].atm;
            if (a[shortest].wt < 0)
            {
                a[shortest].wt = 0;
            }
